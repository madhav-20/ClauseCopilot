...
/* Add breathing room so last messages aren't hidden behind input */
section.main > div {
    padding-bottom: 5rem;
}
</style>
...
with tab2:
    ...
    # ---- (A) Messages area (ALWAYS above input) ----
    messages_placeholder = st.empty()

    def render_messages():
        # Re-render the full message list in one fixed place
    ...
    if prompt:
        ...
        # 2) Immediately refresh messages so user sees their message instantly ABOVE input
        render_messages()
        ...
        try:
            with st.spinner("Thinking..."):
                ...
                history_str = "\n".join(
                    [f"{m['role']}: {m['content']}" for m in st.session_state["chat_messages"][-4:]]
                )

                response = run_chat(model_name, context_text, history_str, prompt)

                # 4) Store assistant message WITH context (so View Context works even later)
                st.session_state["chat_messages"].append(
                    {"role": "assistant", "content": response, "context": context_text}
                )

            # 5) Refresh messages again so assistant answer appears ABOVE input
            render_messages()
...
with tab4:
    ...
    if query:
        try:
            ...
            dists = res.get("distances", [[]])[0]  # optional, may exist depending on Chroma

            if not docs:
                st.warning("No matching clauses found. Try another query.")
                st.stop()

            # Optional: remove repeated near-duplicate docs
            seen = set()
            ...
            for idx, (doc, meta) in enumerate(zip(docs, metas), start=1):
                ...
                # If vendor filter is applied, don’t repeat vendor in every card
                title_left = section if vendor_filter else f"{vendor} • {section}"
                ...
                with st.container(border=True):
                    ...
                    st.caption(f"Result #{idx}" + (f" • Distance: {dists[idx-1]:.3f}" if dists and idx-1 < len(dists) else ""))
